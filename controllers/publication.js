...
function savePublication(req,res){
    ...
    publication.save((err,publicationStored) =>{
        ...
        if(!publicationStored) return res.status(404).send({message:"Publicacion no guardada"})
...
function deletePublication(req,res){
    ...
    Publication.find({user:req.user.sub, '_id':publicationId}).remove(err =>{
        if(err) return res.status(500).send({message:"Error al borrar publicacioens"})
...
function uploadImage(req,res){
    ...
    if(req.files){
        var file_path = req.files.image.file_path
            ...
            Publication.findOne({'user':req.user.sub, '_id':publicationId}).exec((err,publication) =>{
                if(publication){
                    Publication.findByIdAndUpdate(publicationId, {file: file_name},{new:true},(err,publicationUpdated) =>{
                        ...
                        if(!publicationUpdated) return res.status(404).send({message:"No se ha podido actualizar el usario"})
